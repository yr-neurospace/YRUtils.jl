...
Launch external commands contained in `f` concurrently/asynchronously.

To achieve this goal, you must set `wait = false` when calling the `run` function.

Apply `f` to each element of `arr` with optional arguments `f_args`.
...
```
"""
function para_cmds(f::Function, arr::AbstractArray, f_args::Union{NamedTuple,Nothing}=nothing; num_jobs::Int=1, poll_secs::Real=1, verbose::Bool=true)
    arr = copy(arr)
...
"""
    find_cmd(cmd_name::AbstractString; return_nothing::Bool=true, verbose::Bool=true) -> String
...
"""
function find_cmd(cmd_name::AbstractString; return_nothing::Bool=true, verbose::Bool=true)
    cmd_path = ""
    try
...
You can redirect the final `stdout` to `final_stdout_file`.
...
```
"""
function pipe_stderr_one(cmds::Vector{String}, stderr_file::AbstractString; final_stdout_file::AbstractString="")
    cmd_num = length(cmds)
    if cmd_num < 2
...
julia> cmd_valid(`pig --version`)
┌ Error: cmd is invalid
└ @ Main ~/mywd/proj/YRUtils.jl/src/ShellUtils/base_utils.jl:191
